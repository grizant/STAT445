...
the `results_2$no.drug` and `results_2$drug` vectors. We have
...
in the mean values of `no_drug` between `response_1` and `response_2`,
the absolute difference in the mean values of `drug` between
`response_1` and `response_2`, and the absolute difference in mean
values of `no_drug` and `drug` in `response_1`. Of these three
...
the setup, and this question is a prelude to the homework questions.
Here is the new setup: suppose we start with `n=5` subjects (as
...
Use Monte Carlo integration to estimate the following double integral. 

- **4d.** $\int_0^1 \int_0^1 cos(x-y) dx dy$
