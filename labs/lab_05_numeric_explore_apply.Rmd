...
- **2b.** Using `crime_mat_noregion`, compute the maximum value in each of the 5 columns, in two ways. For the first way, use `apply()`, in just one line of code, and call the result `max_vector`. For the second, use a `for()` loop, and call the result `max_vector2`. Use `all.equal()` to show that `max_vector` and `max_vector2` are equal. Hint: this is a bit tricky because you'll need to add names to `max_vector2` in order to get `all.equal()` to return `TRUE` (or use the `use.names` parameter).
